...
| 점수 | 판정 | 조치 |
|------|------|------|
| 90점 이상 | **PASS** | Phase 3 진행 가능 |
| 70~89점 | **CONDITIONAL** | Fixer 수정 후 재검증 |
| 70점 미만 | **FAIL** | Fixer 수정 후 재검증 |
| API 연동 10점 이하 | **AUTO-FAIL** | 점수 무관 자동 불합격 |

### 라운드 규칙
- Tester → Fixer를 1라운드로 센다
- **최대 7라운드**까지 반복
- 매 라운드 Fixer 수정 완료 후 **반드시 커밋**한다
- 7라운드 후에도 PASS(90점) 미달 시 오케스트레이터에게 보고하고 사용자 판단 위임
